        ...
        <div class="bg-white rounded-3xl shadow-2xl overflow-hidden border border-gray-100">
            <div class="bg-gradient-to-r from-emerald-600 to-emerald-700 px-8 py-6">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-white">Edit Transaksi</h1>
                        <p class="text-emerald-100 text-sm">Perbarui data pemasukan atau pengeluaran desa</p>
                    </div>
                </div>
            </div>
            
            <form action="{{ route('admin.keuangan.update', $transaksi->id_transaksi) }}" method="POST" enctype="multipart/form-data" class="p-8">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <label class="relative cursor-pointer">
                                <input type="radio" name="jenis" value="pemasukan" {{ $transaksi->jenis == 'pemasukan' ? 'checked' : '' }} class="sr-only peer" required>
                            ...
                            <label class="relative cursor-pointer">
                                <input type="radio" name="jenis" value="pengeluaran" {{ $transaksi->jenis == 'pengeluaran' ? 'checked' : '' }} class="sr-only peer" required>
                                <div class="p-5 border-2 rounded-xl text-center transition-all peer-checked:border-emerald-500 peer-checked:bg-emerald-50 hover:border-emerald-300 hover:shadow-md">
                    ...
                    <div>
                        ...
                        <textarea name="deskripsi" rows="3" required
                                  ...
                                  placeholder="Contoh: Pembangunan jalan desa, Pembayaran gaji perangkat desa, dll">{{ old('deskripsi', $transaksi->deskripsi) }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="jumlah" step="1000" value="{{ old('jumlah', $transaksi->jumlah) }}" required
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">
                            <i class="fa-solid fa-camera text-emerald-500 mr-1"></i> Bukti Transaksi
                        </label>
                        <div class="border-2 border-dashed border-gray-300 rounded-xl p-6 text-center hover:border-emerald-400 transition">
                            @if($transaksi->bukti_foto)
                                <div class="mb-3">
                                    <img src="{{ asset('storage/' . $transaksi->bukti_foto) }}" class="max-h-32 mx-auto rounded-lg mb-2">
                            ...
                            <label for="bukti_foto" class="cursor-pointer flex flex-col items-center gap-2">
                                ...
                                <span class="text-sm text-gray-500">{{ $transaksi->bukti_foto ? 'Ganti foto' : 'Upload bukti transaksi' }}</span>
                ...
                <div class="mt-8 flex gap-4 justify-end">
                    ...
                    <button type="submit" class="px-6 py-3 bg-gradient-to-r from-emerald-600 to-emerald-700 text-white rounded-xl font-semibold hover:shadow-lg transition flex items-center gap-2">
                        <i class="fa-solid fa-save"></i> Update Transaksi
                    </button>
                </div>
            </form>
        </div>
